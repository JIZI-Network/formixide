...
use crate::error::{AuthorizationError, StorageError};
...
pub trait FormEngine<T: Sync> {
    fn get_authorizer(&self) -> &impl Authorizer<T>;
    ...
    async fn create_form(
        ...
        if self.get_authorizer().can_create_form(t, &form_definition) {
            ...
            Err(FormEngineError::AuthorizationError(AuthorizationError {
    ...
    async fn load_forms_by_role(
        ...
        if self.get_authorizer().can_read_forms_by_role(t, role) {
